...
def createMap():
    ...
    polygon_boundary(img)
    boomerang_boundary(img)
    circle_boundary(img)
...
def circle_boundary(img): # 5mm proximity
    cv2.circle(img,(300,65), 40, (255,255,255), thickness= 3)

# Creating Polygon as an obstacle on the map
def polygon_boundary(img): # 5mm proximity
    hexa = np.array([[[200,190],[235,170],[235,130],[200,110],[165,130],[165,170]]], np.int32)
    cv2.polylines(img, [hexa],True, (255,255,255), thickness=3)
...
def boomerang_boundary(img): #5mm proximity
    hexa = np.array([[[36,65],[115,40],[80,70],[105,150]]], np.int32)
    cv2.polylines(img, [hexa], True, (255,255,255), thickness=3)
